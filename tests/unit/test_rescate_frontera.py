...
# --- Simplificar no puede perder gente -------------------------------------


@pytest.mark.geo
def test_simplificar_la_frontera_no_pierde_celdas() -> None:
    """La garantia de la simplificacion: una celda cambia de dueno, no se pierde.

    Es lo que hace aceptable el recorte a 55 m. El reparto asigna por el centro
    de la celda y una celda r8 mide unos 740 m, asi que mover una linea 55 m
    solo puede afectar a las celdas que caen casi encima de ella — y esas pasan
    al municipio vecino, no al vacio. El total nacional no se mueve.
    """
    from pipelines.p0_exposure.crosswalk import SIMPLIFICACION_ADMIN_GRADOS
    from pipelines.p2_impact.exposure_join import connect

    con = connect()
    # Dos municipios pegados, con una frontera compartida llena de zigzags de
    # amplitud muy inferior a la tolerancia.
    dientes = ", ".join(f"1.0000{i % 2} {i / 200:.4f}" for i in range(200))
    izq = f"POLYGON((0 0, {dientes}, 1 1, 0 1, 0 0))"
    der = f"POLYGON((1 1, {dientes}, 2 0, 2 1, 1 1))"
    con.execute(
        "CREATE TABLE bruto AS "
        f"SELECT 'A' AS adm2_id, ST_GeomFromText('{izq}') AS geom "
        f"UNION ALL SELECT 'B', ST_GeomFromText('{der}')"
    )

    def celdas(tolerancia: float) -> int:
        expr = f"ST_SimplifyPreserveTopology(geom, {tolerancia})" if tolerancia else "geom"
        return int(
            con.execute(
                "SELECT count(DISTINCT h3) FROM ("
                f"  SELECT unnest(h3_polygon_wkt_to_cells(ST_AsText({expr}), 8)) AS h3"
                "   FROM bruto)"
            ).fetchone()[0]
        )

    sin_tocar = celdas(0.0)
    simplificado = celdas(SIMPLIFICACION_ADMIN_GRADOS)
    assert sin_tocar > 0
    # La cobertura total no cae: como mucho una celda cambia de municipio.
    assert simplificado >= sin_tocar * 0.999, (
        f"la simplificacion perdio celdas: {sin_tocar} -> {simplificado}"
    )
